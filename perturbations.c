...
int perturbations_derivs(double tau,
  ...
  double Ansatz_Q, Ansatz_deltaM, Ansatz_psiM, Ansatz_psiM, Ansatz_psiX, Ansatz_thetaX;
  ...
  Ansatz_psiM = (a * Ansatz_Q * pvecmetric[ppw->index_mt_psi])/ppw->pvecback[pba->index_bg_rho_cdm];  /**  pvecmetric[ppw->index_mt_psi]**/
  Ansatz_thetaX = (a * Ansatz_Q * y[pv->index_pt_theta_cdm])/pvecback[pba->index_bg_rho_fld];
  Ansatz_psiX = (a * Ansatz_Q * pvecmetric[ppw->index_mt_psi])/pvecback[pba->index_bg_rho_fld];
  /** Ansatz_dQ = Concluindo **/
  ...
  /** ##################################### MUDEI AQUI ################################ **/
      ...
      if (ppt->gauge == newtonian) {
        dy[pv->index_pt_delta_cdm] = -(y[pv->index_pt_theta_cdm]+metric_continuity) - Ansatz_deltaM + Ansatz_psiM + a*Ansatz_dQ/ppw->pvecback[pba->index_bg_rho_cdm]; /* cdm density */
  ...
  /** ###################################### MUDEI AQUI ###################################### **/
       ...
       /** dy[pv->index_pt_delta_fld] =
        ...
        dy[pv->index_pt_delta_fld] = -3*a_prime_over_a*y[pv->index_pt_delta_fld]*(1+w_fld) + 3*(1+w_fld)*pvecmetric[ppw->index_mt_phi_prime]
             (1 + w_fld)*y[pv->index_pt_theta_fld] - Ansatz_psiX - a*Ansatz_dQ/pvecback[pba->index_bg_rho_fld];

        /** - ----> fluid velocity */

        dy[pv->index_pt_theta_fld] = (a * Ansatz_Q * y[pv->index_pt_theta_fld])/pvecback[pba->index_bg_rho_fld]
           -(3*w_fld -1)*a_prime_over_a*y[pv->index_pt_theta_fld] + +metric_euler + k2*w_fld*y[pv->index_pt_delta_fld]/(1.+w_fld)
           - Ansatz_thetaX;
